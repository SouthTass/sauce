 ...
 * @param { string } variety      商品品种
 * @param { string } contract     合约日期
 */
async function getDealPosition(date, variety, contract){
  ...
  if(res.status != 200) return { status: 1, message: `同花顺期货通查询指定品种指定日期持仓情况接口无法访问` }
...
/**
 * 获取散户指定日期分析 东财 徽商 平安 方正中期 中信建投
 * @param { string } date         查询日期
 * @param { string } variety      商品品种
 * @param { string } contract     合约日期
 */
async function getRetailInvestorAnalysis(date, variety, contract, type){
  let list = []
  if(type == '家人'){
    list = ['东方财富', '徽商期货', '平安期货', '方正中期', '中信建投']
  }else if(type == '机构'){
    list = ['国泰君安期货', '海通期货', '华泰期货', '中信期货', '东证期货', '永安期货']
  }else if(type == '外资'){
    list = ['乾坤期货', '瑞银期货', '摩根大通期货']
  }
  let res = await getDealPosition(date, variety, contract)
  dataList = res.list
  let resultText = []
  list.map(e => {
    let itemText = []
    let index = dataList.findIndex(a => a.company == e)
    if(index != -1){
      let item = dataList[index]
      itemText.push(e)
      itemText.push(`净持仓：${item.f024n}，较上日：${item.f025n}`) 
      itemText.push(`多单：${item.f009n}，较上日：${item.f013n}`)
      itemText.push(`空单：${item.f015n}，较上日：${item.f019n}`)
      // if(item.f024n != 0 || item.f025n != 0) itemText.push(`净持仓：${item.f024n}，较上日：${item.f025n}`) 
      // if(item.f009n != 0 || item.f013n != 0) itemText.push(`多单：${item.f009n}，较上日：${item.f013n}`)
      // if(item.f015n != 0 || item.f019n != 0) itemText.push(`空单：${item.f015n}，较上日：${item.f019n}`)
    }
    if(itemText.length > 0) resultText.push(itemText.join('\n'))
  })
  return resultText.join('\n')
}


async function getRank(date, variety, contract){
  let result = []
  result.push('【家人动向】')
  result.push(await getRetailInvestorAnalysis(date, variety, contract, '家人'))
  result.push('\n')
  result.push('【机构动向】')
  result.push(await getRetailInvestorAnalysis(date, variety, contract, '机构'))
  result.push('\n')
  result.push('【外资动向】')
  result.push(await getRetailInvestorAnalysis(date, variety, contract, '外资'))
  console.log(result.join('\n'))
}
getRank('2026-04-28', 'P', 'P2609')
...
// /**
//  * 查询指定品种在指定时间内的盈利情况
//  * @param { string } time         查询多久的数据，如7,15,30天
//  * @param { string } end_date     结束日期
//  * @param { string } variety      商品品种
//  */
// async function position_profit_rank(item){
//   let params = {
//     type: 'variety', //按品种查询，固定值
//     variety: item.variety, //品种代码
//     start_date: dayjs(item.end_date).subtract(item.time, 'day').format('YYYY-MM-DD'), //查询开始时间
//     end_date: dayjs(item.end_date).format('YYYY-MM-DD'),  //查询结束时间 
//   }
//   let res = await axios({
